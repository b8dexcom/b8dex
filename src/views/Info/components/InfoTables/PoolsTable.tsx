...
import {formatAmount, formatAmountThousands} from 'views/Info/utils/formatInfoNumbers'
...
const DataRow = ({ poolData, index, priceUSDTBCC }: { poolData: PoolData; index: number, priceUSDTBCC: number }) => {
    ...
    <LinkWrapper key={index} to={`/pools/${poolData.address}`}>
      <ResponsiveGrid>
        ...
        <Flex alignItems="center">
          ...
          <Text ml="6px" fontSize="15px" color="#505050" fontWeight="normal" lineHeight="16px">TBCC</Text>
        </Flex>
        <Text mr="8px" fontSize="15px" color="#505050" fontWeight="normal" lineHeight="16px">{formatAmount(poolData.lpApr7d)}%</Text>
        <Flex alignItems="center">
          {/* <CurrencyLogo currency={currencyTBCC} size="20px"/> */}
          <Flex p="4px" justifyContent="center"minWidth="80px" background={poolData.volumeUSDChange === 0 ? '#F9F9F9' : poolData.volumeUSDChange < 0 ? '#FFF5EC' : '#DEFFEB'}>
            <Text fontSize="15px" color={poolData.volumeUSDChange === 0 ? '#8A8A8A' : poolData.volumeUSDChange < 0 ? '#FF822E' : '#2DC96C'} fontWeight="normal" lineHeight="16px">{poolData.volumeUSD ? formatAmount(poolData.volumeUSD * priceUSDTBCC) : '—'}</Text>
          </Flex>
          <Text ml="6px" fontSize="15px" color="#505050" fontWeight="normal" lineHeight="16px">TBCC</Text>
        </Flex>
        <Flex alignItems="center">
          ...
          >
            <Area dataKey="value" type="monotone" stroke={poolData.volumeUSDChangeWeek > 0 ? '#2DC96C' : '#FF7D61'} fill="url(#gradient)" strokeWidth={1.5} />
          </AreaChart>
          <Flex mr="16px" />
          {/* <CurrencyLogo currency={currencyTBCC} size="20px"/> */}
  ...
  return (
    <TableWrapper>
      <ResponsiveGrid style={{ paddingBottom: "12px" }}>
        <Text color="#8A8A8A" fontSize="15px" lineHeight="16px" fontWeight="normal">
          {t('Cryptocurrency')}
        </Text>
        ...
        >
          {t('Price1')} {arrow(SORT_FIELD.token1Price)}
        </ClickableColumnHeader>
        <ClickableColumnHeader
          color="#8A8A8A"
          fontSize="15px"
            ...
            if (poolData) {
              return (
                <React.Fragment key={poolData.address}>
                  <DataRow index={i} poolData={poolData} priceUSDTBCC={priceUSDTBCC} />
                </React.Fragment>
              )
            }
            return null
