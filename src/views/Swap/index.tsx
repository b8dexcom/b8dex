...
import { Button, Text, Box, Flex, useMatchBreakpoints, Tab, TabMenu } from '../../uikit'
...
import { useExpertModeManager, useUserSlippageTolerance, useUserSingleHopOnly, useExchangeChartManager } from '../../state/user/hooks'
...
export default function Swap({ history }: RouteComponentProps) {
  ...
  const [userChartPreference] = useExchangeChartManager(isMobile)
  ...
  return (
    <Page removePadding={isChartExpanded} hideFooterOnDesktop={isChartExpanded}>
      {isMobile && (
        <TabMenu activeIndex={mobileTab} onItemClick={handleMobileTabClick}>
          <Tab>{t('Exchange')}</Tab>
          <Tab>{t('Charts')}</Tab>
        </TabMenu>
      )}

      <Flex width="100%" justifyContent={isChartDisplayed ? 'space-between' : 'center'} position="relative">
        {mobileTab === 0 ? (
          <Flex flexDirection="column" width={isMobile ? '100%' : 'auto'} >
            <StyledSwapContainer>
              ...
              >
                <AppBody boxShadow="0px 8px 16px rgba(0, 0, 0, 0.02)">
                  <CurrencyInputHeader
                    title={t('Swap')}
                    setSwitchTokens={setSwitchTokens}
