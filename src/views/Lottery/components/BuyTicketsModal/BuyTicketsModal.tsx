...
import { useB8D, useLotteryV2Contract} from 'hooks/useContract'
...
const EditButton = styled(Button)`
  ...
  -webkit-filter: drop-shadow(0px 8px 16px rgba(38,219,148,0.15));
...
const BuyTicketsModal: React.FC<BuyTicketsModalProps> = ({ onDismiss }) => {
  ...
  const b8dContract = useB8D()
  const { toastSuccess } = useToast()
  const { balance: userCake, fetchStatus } = useTokenBalance(tokens.b8d.address)
  ...
  useEffect(() => {
    const getMaxPossiblePurchase = () => {
      const maxBalancePurchase = memoisedUserCake.div(priceTicketInTBCC)
  ...
  const { isApproving, isApproved, isConfirmed, isConfirming, handleApprove, handleConfirm } =
    useApproveConfirmTransaction({
      onRequiresApproval: async () => {
        try {
          const response = await b8dContract.allowance(account, lotteryContract.address)
      ...
      onApprove: () => {
        return callWithGasPrice(b8dContract, 'approve', [lotteryContract.address, ethers.constants.MaxUint256])
      },
  ...
  return (
    <StyledModal title={t('Buy Tickets')} onDismiss={onDismiss} headerBackground={theme.colors.gradients.cardHeader}>
      ...
      <BalanceInput
        ...
        currencyValue={
          cakePriceBusd.gt(0) &&
          `~${ticketsToBuy ? getFullDisplayBalance(priceTicketInTBCC.times(new BigNumber(ticketsToBuy))) : '0.00'} B8D`
        }
      />
      ...
      <Flex flexDirection="column">
        <Flex mb="8px" justifyContent="space-between">
          ...
          <Text color="textSubtle" fontSize="14px">
            {priceTicketInTBCC && getFullDisplayBalance(priceTicketInTBCC.times(ticketsToBuy || 0))} B8T
          </Text>
        </Flex>
        <Flex mb="8px" justifyContent="space-between">
          <Flex>
            ...
            {isApproved && (
              <EditButton
                ...
                endIcon={
                  <ArrowForwardIcon
                    ml="2px"
                    color={disableBuying || isConfirming ? '#bdc2c4' : '#4E89E3'}
                    height="24px"
                    width="24px"
                  />
                }
