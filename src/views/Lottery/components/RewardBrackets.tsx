...
const RewardBrackets: React.FC<RewardMatchesProps> = ({ lotteryNodeData, isHistoricRound }) => {
  ...
  useEffect(() => {
    if (lotteryNodeData) {
      ...
      const cakeToBurn = feeAsPercentage.div(100).times(new BigNumber(amountCollectedInTBCC))
      const amountLessTreasuryFee = new BigNumber(amountCollectedInTBCC).minus(cakeToBurn)
      setState({
        isLoading: false,
        cakeToBurn,
